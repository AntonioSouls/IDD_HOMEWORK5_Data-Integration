...
def create_schema_mapping(json_file_path):
    attributes_data = read_file(json_file_path)
    
    # Define the prompt
    prompt = "".join(open("data/messageForModelSchemaMapping.txt", "r", encoding='utf-8').readlines())
    ...
    with open('data/schema_mapping.json', 'w', encoding='utf-8') as json_file:
        json_file.write(json_response)

# Main function
def main():
    json_file_path = 'data/attributes_descriptions.json'
    output_file = 'data/schema_mapping.json'
    ...
    create_schema_mapping(json_file_path)
